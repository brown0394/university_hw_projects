...
task_t cyclic_tasks[TASK_N] = {
   {"Task_500ms" , AppTask_500ms,  2, &Task_500ms_Stack[0] , &Task_500ms_TCB},
   {"Task_1000ms", AppTask_1000ms, 2, &Task_1000ms_Stack[0], &Task_1000ms_TCB},
   {"Task_2000ms", AppTask_2000ms, 2, &Task_2000ms_Stack[0], &Task_2000ms_TCB},
...
*                                          AppTask_USART
*
* Description : Used to read input and echo back with USART
*
* Arguments   : p_arg (unused)
*
* Returns     : none
    ...
    while (DEF_TRUE) {                                          /* Task body, always written as an infinite loop.       */
    	while (USART_ReceiveData(Nucleo_COM1) != '`') {
    		while(USART_GetFlagStatus(Nucleo_COM1, USART_FLAG_RXNE) == RESET){}
    		 USART_SendData(Nucleo_COM1, USART_ReceiveData(Nucleo_COM1));
    		 receivePrinted = 0;
    	}
    	if (!receivePrinted) {
    		receivePrinted = 1;
