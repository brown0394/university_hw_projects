...
function createBoxNode(ith) {
    var newNode = document.createElement("button");
    newNode.setAttribute("id", "b"+ith);
    newNode.setAttribute("onclick", "showInfo("+ith+")");
    newNode.appendChild(document.createTextNode(storedArr[ith].i));
    return newNode;
}

function createIndexNode(ith) {
    var newNode = document.createElement("input");
    newNode.setAttribute("id", "s"+ith);
    newNode.setAttribute("type", "button");
    newNode.setAttribute("onclick", "selectIndex("+ith+")");
    newNode.setAttribute("value", ith+1);
    return newNode;
}
...
function drawBox(base) {
    ...
    for (var i = base; i < storedArr.length && i < (base+10); ++i){
        fbox.appendChild(createBoxNode(i));
    }
    //iSel.innerHTML = "";
    isel.textContent = "";
    if (storedArr.length > 10) {
        ...
        for (var j = 0; j < numbers; ++j) {
            isel.appendChild(createIndexNode(j));
        }
        var indexButton;
        if (base > 0) {
...
function replaceText(parent, text) {
...
function showInfo(n) {
    if (n >= 0) {
        let got = storedArr[n];
        replaceText(pArr[0], got.n);
        replaceText(pArr[1], got.p);
        if (got.e == "") {
            replaceText(pArr[2], "-");
        }
        else {
            replaceText(pArr[2], got.e);
        }
        if (got.m == "") {
            replaceText(pArr[3], "-");
        }
        else {
            replaceText(pArr[3], got.m);
        }
      //  ibox.style.display = "block";
        ibox.setAttribute("style", "display : block");
    }
}
